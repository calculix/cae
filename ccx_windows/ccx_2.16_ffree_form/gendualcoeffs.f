...
!
!
! >    \brief Determining the coefficients of the dual shape functions on
! >       the slave surface
...
! > @param   [in]     iinc       index increment
! > @param   [in]     iit        index iteration
...
! > @param   [in]     pslavsurf  integration points and weights on slave side
      ...
      do i=1,ntie
         ...
         do l = itiefac(1,i), itiefac(2,i)
            ...
            call getnumberofnodes(nelems,jfaces,lakon,nope,&
                 nopes,idummy)
            !             checkbiorthogonality=.false.
            !             if(nelems.eq.39816 .or.nelems.eq.40364) then
            !               checkbiorthogonality=.true.
            !             endif
            !
           ...
           if(checknorm)then
            ...
            !             write(*,*)'pslavdual1, face',l,'nnogap set to 0',nnogap
            !             nnogap=0
            !
            ...
            if(checknorm)then
               ...
               do ii=1,8
                  ...
                  do j=1,8
                     help=help+pslavdual((ii-1)*8+j,l)
                  enddo
                  !      write(*,105) (pslavdual((ii-1)*8+j,l),j=1,nopes),help
 ...
 105        format(8(1x,e15.4))
         ...
         if(checkbiorthogonality)then
            do l = itiefac(1,i), itiefac(2,i)
               ...
               if(mint2d==0) cycle
               indexf=islavsurf(2,l)
               !      write(*,*)'face',l,'indexf',indexf,mint2d,i
               do m=1,mint2d
                  !                   write(*,*)'m',m+indexf
                  !      write(*,*)'psl',pslavsurf(1,indexf+m),
                  !      &                 pslavsurf(2,indexf+m),pslavsurf(3,indexf+m)
                  xis=pslavsurf(1,indexf+m)
                  ets=pslavsurf(2,indexf+m)
                  weight=pslavsurf(3,indexf+m)
                  ns=l
                  iflag = 2
 !      write(*,100) xis, ets,weight
                  ...
                  do j=1,nopes
                     ...
                     do jj=1,nopes
                        ...
                        contribution=shp2s(4,locs)*shp2m(4,locm)&
                             *weight&
                             *dx
                        !      if(m==1)write(*,*)'i,j',j,jj,8*(j-1)+jj
