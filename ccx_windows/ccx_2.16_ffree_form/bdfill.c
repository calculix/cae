...
/**
 * \brief Calculates the coupling matrices \f$ B_d \f$ and \f$ D_d \f$, and insert them into the data structure
 ...
 * @param [out] nzsbd		nonzero entries of aubd 
 ...
 * @param [out] nzsbdtil	nonzero entries of aubdtil 
 ...
 * @param [out] nzsbdtil2	nonzero entries of aubdtil2 
 ...
 * @param [in] inoels		...which stores 1D&2D elements belonging to node: (1,i)el. number (2,i) # nodes (3,i) pointer to next entry 
 ...
 * @param [out] gap		(i) \f$ g_i= <g, \Psi_i> \f$ for node i on slave surface
 ...
 * @param [in] nmastmpc2	(2*i) pointer to imastmpc2...
 * @param [in] imastmpc2	... which stores transformed MPCs for master node i
 * @param [in] nmmpc2		number of MPC for master nodes 
...
void bdfill(ITG **irowbdp, ITG *jqbd,double **aubdp,ITG *nzsbd,
  ...
  ITG i, ii,j,jj, k,kk,l,m, ll,icounter,icounter2,idof1,idofs,idofm, nodesf, nodem, kflag,numb,isn,imn,
    *mast1=NULL,nzsbhelp,idof0,
    number, *irowbd=NULL,*irowdd=NULL,*irowddinv=NULL,*irowddtil=NULL,*irowddtilinv=NULL,*jqddtilinv=NULL,
    *irowbdtil=NULL,*irowbdtil2=NULL,*irowddinvt=NULL,*jqddinvt=NULL,*irowddtil2=NULL,
    ifree,ifreedd,ifreeddinv,ifreeddtil,nslavdof,ifree1,ifree3,
    mt=mi[1]+1,istart, *iscontr=NULL, *imcontr=NULL,*igap=NULL,
    *idcontr1=NULL, *idcontr2=NULL,*igcontr=NULL, debug,intpointl, *mast2=NULL,ifree2,
    ist,ist2,dir,node1,jslav,dim,id,dof,index,dirdep,dirind,index2,id2,dirdep2,dirind2,node2,idof2,nzsbd2,
    idof3,islavk2,nzsddtil,nzsddinv,nzsdd,nzstloc,nzstlocinv,*irowbdt=NULL,*jqbdt=NULL,imastk2,*irowb=NULL,
    nzsb1,nzsb2,nzsbtil2,nzsdtil,nzsd,nzsdtilinv,nzsb,nzsdinv,nzsbtil,nzsbtil1,nzsd1,nzsb2a,
    *mast3=NULL, *irowdtil=NULL,*irowb1=NULL,*jqb1=NULL,*irowb2=NULL,*jqb2=NULL,*irowbtil2=NULL,*jqbtil2=NULL,
    *irowd=NULL,*irowbtil=NULL,*irowdinvt=NULL,*jqdinvt=NULL,
    *irowdinv=NULL,*jqdinv=NULL,*jqbtil1=NULL,*irowbtil1=NULL,*jqb2a=NULL,*irowb2a=NULL,*jqd1=NULL,*irowd1=NULL,*irowbhelp=NULL,
    *irowbtil2t=NULL,*jqbtil2t=NULL,*irowdt=NULL,*jqdt=NULL,nzsdt,*irowdtil2=NULL,*jqdtil2=NULL,nzsddtil2,nzsdtil2,
    islavnodeentry,jrow,jslavnodeentry,iadd,
    *irowdpg=NULL,*irowbpg=NULL,*irowdpgtil=NULL,*irowbpgtil=NULL,ifreepg1,ifreepg2,ifreepg3,nzsbpg1,nzsbpg2,nzsbpgtil2,
    *mastpg1=NULL,*mastpg2=NULL,*mastpg3=NULL,nzsbpg2a,nzsdpg,nzsbpg,nzsdpgtil,nzsbpgtil,nzsdpg1,
    *irowbpg1=NULL,*jqbpg1=NULL,*irowbpg2=NULL,*jqbpg2=NULL,*irowbpgtil2=NULL,*jqbpgtil2=NULL,*irowdpg1=NULL,*jqdpg1=NULL,
    *irowdpgtilt=NULL,*jqdpgtilt=NULL,*irowbpg2a=NULL,*jqbpg2a=NULL,*irowbpgtil2t=NULL,*jqbpgtil2t=NULL;
  
  double contribution=0.0, *aubd=NULL, *audd=NULL, *auddtil=NULL, *auddtil2=NULL, *auddinv=NULL,*Bdhelp=NULL,
    *contr=NULL,*dcontr=NULL, *gcontr=NULL,*anull=NULL, *Bd=NULL,*auddtilinv=NULL,*aubdtil=NULL,*aubdtil2=NULL,*auddinvt=NULL,
    gap2,*xs=NULL,*xm=NULL,dm[3],n[3],c2,coefdep,coefind,coefdep2,coefind2,c3,detdh,dh[9],*aubdt=NULL,
    *gtil=NULL,*Dd=NULL,*Ddinv=NULL,*Bdtil1=NULL,*Ddt=NULL,*Ddtil2=NULL,help[3],*dinvloc=NULL,*dloc=NULL,
    *Bd1=NULL,*Bd2=NULL,*Bdtil2=NULL,*Bdtil=NULL,*Ddtil=NULL,*Ddinvt=NULL,*Dd1=NULL,*Bd2a=NULL,*Bdtil2t=NULL,
    detdloc,e1,e2,e3,
    *Bpgd=NULL,*Dpgd=NULL,*Bpgdtil=NULL,*Dpgdtil=NULL,*Bpgd1=NULL,*Bpgd2=NULL,*Bpgdtil2=NULL,*Dpgd1=NULL,*Dpgdtilt=NULL,
    *Bpgd2a=NULL,*Bpgdtil2t=NULL; 
  /*
  ...
  ifreedd=1; ifreeddtil=1; ifreeddinv=1;
  ...
  // @TODO: make martix multiplications more effective
  //       store in form of multimortar?
  debug=0;
  ...
  */
  ...
  for( i=0; i<*ntie; i++){    
	...
	//debug=1;
	if(debug==1)printf("bdfill face %" ITGFORMAT " intpoints %" ITGFORMAT " %" ITGFORMAT " \n",l, islavsurf[2*(l-1)+1],islavsurf[2*l+1]);
	//debug=0;
	...
	if(intpointl>0){
	  ...
	  //printf("bdfill: dimensions %" ITGFORMAT " \n",9*9*intpointl/7+1 );
	  // build dual coupling matrices (see phd-thesis Sitzmann equation (4.18) and (4.19))
	  FORTRAN(createbd,(&i,&l,ipkon,kon,lakon,co,vold,gapmints,
			    ...
			    nslavnode,islavnode,nmastnode,imastnode,&icounter,&icounter2,islavact,iflagdualquad));
	  /* build B_d **/
	  for(j=0; j<icounter;j++){				
	    ...
	    if ((contribution>1e-14 ||contribution<-1e-14)){ 
	      //if(debug==1)printf("\tbdfill: B_d nodesf %" ITGFORMAT " nodem %" ITGFORMAT " c %e \n",nodesf,nodem,contribution);
	  ...
	  for(j=0; j<icounter2;j++){			
	    ...
	    if(nodesf==nodem){
	      ...
	      if(contribution>1e-14 ||contribution<-1e-14){
		if(debug==1)printf("\tbdfill: face %" ITGFORMAT " node %" ITGFORMAT " %" ITGFORMAT " Ddtil %e\n",l,nodesf,nodem,contribution);
		insertas(&irowdtil, &mast1,&nodesf, &nodem,  &ifree1, &nzsdtil,
	      ...
	      if(contribution>1e-14 ||contribution<-1e-14){	
		if(debug==1){
		  printf("\tbdfill: face %" ITGFORMAT " node %" ITGFORMAT " %" ITGFORMAT " Bdtil2 %e\n",l,nodesf,nodem,contribution);
		}
		// Bbtil2^T
		// ... are stored in Bdtil
	  ...
	  // calculate additional PG matrices (see phd-thesis Sitzmann equation (4.20) and (4.21))
	  if(*iflagdualquad>2){
	  ...
	  FORTRAN(createbd,(&i,&l,ipkon,kon,lakon,co,vold,gapmints,
			    islavsurf,imastsurf,pmastsurf,contr,iscontr,imcontr,
			    dcontr,idcontr1,idcontr2,gcontr,igcontr,iponoels,inoels,mi,pslavsurf,pslavdualpg,
			    nslavnode,islavnode,nmastnode,imastnode,&icounter,&icounter2,islavact,iflagdualquad));
	  /* build Bpg_d **/
	  for(j=0; j<icounter;j++){				
	    contribution=-contr[j];				
	  ...
	  for(j=0; j<icounter2;j++){			
	    ...
	    if(islavact[idcontr1[j]-1]>-1){ 
	      if(contribution>1e-14 ||contribution<-1e-14){
		if(debug==1)printf("\tbdfill: face %" ITGFORMAT " node %" ITGFORMAT " %" ITGFORMAT " Ddtil %e\n",l,nodesf,nodem,contribution);
		insertas(&irowdpgtil, &mastpg1,&nodesf, &nodem,  &ifreepg1, &nzsdpgtil,
			 &contribution, &Dpgdtil);	
	      }
	    }else{  	      
	      if(contribution>1e-14 ||contribution<-1e-14){	
		if(debug==1){
		...
		// Bpgdtil2^T
		insertas(&irowbpgtil2, &mastpg3,&nodem, &nodesf,  &ifreepg3, &nzsbpgtil2,
			 &contribution, &Bpgdtil2);
	      }						
	    }		        
	  }
	  }
  ...
  for (i=0;i<*ntie;i++){	  
    if(tieset[i*(81*3)+80]=='C'){	    
      for(j=nslavnode[i];j<nslavnode[i+1];j++){
	if(igap[j]<1){islavact[j]=-3;}
	//if(islavact[j]>-1)printf("\t node %d gap %e \n",islavnode[j],gap[j]);
      }
    }
  }
  ...
  printf("\tbdfill: \tsize Ddtil %" ITGFORMAT " \n",icounter);
  nzsdtil=icounter;
  RENEW(irowdtil,ITG,nzsdtil);
  RENEW(Ddtil,double,nzsdtil); 
  SFREE(mast1);
  ...
  jqb1[*nk]=icounter+1; 
  printf("\tbdfill: \tsize Bd1 %" ITGFORMAT " \n",icounter);
  nzsb=icounter;
  RENEW(irowb1,ITG,nzsb1);
  RENEW(Bd1,double,nzsb1); 
  SFREE(mast2);
  /*for(i=0;i<*nk;i++){
    for(j=jqb1[i]-1;j<jqb1[i+1]-1;j++){
    printf("B1 c %" ITGFORMAT " r %" ITGFORMAT " v %e \n",i+1,irowb1[j],Bd1[j]); 
    }
    }  */
  /* sort Bdtil2 **/
  nzsbtil2=ifree3-1;
  ...
  printf("\tbdfill: \tsize Bdtil2 %" ITGFORMAT " \n",icounter);
  nzsbtil2=icounter;
  ...
  if(*iflagdualquad>2){
    ...
    printf("\tbdfill: \tsize Dpgdtil %" ITGFORMAT " \n",icounter);
    nzsdtil=icounter;
    RENEW(irowdpgtil,ITG,nzsdpgtil);
    RENEW(Dpgdtil,double,nzsdpgtil); 
    SFREE(mastpg1);
    ...
    printf("\tbdfill: \tsize Bpgd1 %" ITGFORMAT " \n",icounter);
    nzsb=icounter;
    ...
    printf("\tbdfill: \tsize Bpgdtil2 %" ITGFORMAT " \n",icounter);
    nzsbpgtil2=icounter;
  ...
  printf("\tbdfill: \tinverse of Dtil calculated %" ITGFORMAT "\n",ifree2-1);	  
  
  nzsbtil=nzsb1;
  RENEW(Bdtil,double,nzsbtil);
  RENEW(irowbtil,ITG,nzsbtil);
  // Bd on master side plus contributions from slave nodes without LM contribution (no-gap and no-LM) 
  add_rect(Bd1,irowb1,jqb1,dim,dim,
	   Bdtil2t,irowbtil2t,jqbtil2t,dim,dim,
	   &Bdtil,&irowbtil,jqbtil,&nzsbtil);
  
  printf("\tbdfill: \tsize Bdtil %" ITGFORMAT " \n",jqbtil[*nk]-1);
  ...
  if(*iflagdualquad>2){
    ...
    printf("\tbdfill: \tsize Bpgdtil %" ITGFORMAT " \n",jqbpgtil[*nk]-1); 
  }
  ...
  for( i=0; i<*ntie; i++){    
	...
	for(ii=0;ii<*nk;ii++){
	  for(jj=jqd[ii]-1;jj<jqd[ii+1]-1;jj++){
	    if(irowd[jj]==nodesf){
	      ...
	      //printf("\t r %" ITGFORMAT " c %" ITGFORMAT " dd %e \n",irowd[jj],ii+1,Dd[jj]);
	      }
	  }
	}
	for(ii=0;ii<*nk;ii++){
	...
	if(contribution<-1.e-10 ||contribution>1.e-10){
	  printf("\tbdfill: node %" ITGFORMAT " sum Dd+Bd %e \n",nodesf,contribution);
	}
	}
      }
    }
  ...
  for( i=0; i<*ntie; i++){    
	...
	if(contribution<-1.e-10 ||contribution>1.e-10){
	  printf("\tbdfill: node %" ITGFORMAT " sum Ddtil+Bdtil %e \n",nodesf,contribution);
	}
	}
      }
    }
  ...
  for( i=0; i<*ntie; i++){    
	...
	if(contribution<-1.e-10 ||contribution>1.e-10){
	  printf("\tbdfill: node %" ITGFORMAT " sum Dpgd+Bpgd %e \n",nodesf,contribution);
	}
	}
      }
    }
  ...
  for( i=0; i<*ntie; i++){    
	...
	if(contribution<-1.e-10 ||contribution>1.e-10){
	  printf("\tbdfill: node %" ITGFORMAT " sum Dpgdtil+Bpgdtil %e \n",nodesf,contribution);
	}
	}
      }
    }
  ...
  printf("\tbdfill: \tsize Bdhelp %" ITGFORMAT " \n",jqbhelp[*nk]-1);
  
  /* Ddtil2=Dd^-1*D^T= ID **/
  /* this is only help matrix needed for changed procedure dealing with quadratic elements */
  ...
  for(ii=0;ii<*nk;ii++){
    for(jj=jqdtil[ii]-1;jj<jqdtil[ii+1]-1;jj++){
      //        k=irowdtil[jj]-1;
      //	if(jqdtilinv[k+1]-jqdtilinv[k]==1){
      Ddtil2[jj]=1.0;
  ...
  /*for(ii=0;ii<*nk;ii++){
    for(jj=jqtloc[ii]-1;jj<jqtloc[ii+1]-1;jj++){
    printf("Ddtil2 c %" ITGFORMAT " r %" ITGFORMAT " v %e \n",ii+1,irowtloc[jj],autloc[jj] );
    }
    }*/
  
  
  printf("\tbdfill: \tsize Ddtil2 %" ITGFORMAT " \n",jqdtil2[*nk]-1);
  if(debug==1){
  for( i=0; i<*ntie; i++){    
    if(tieset[i*(81*3)+80]=='C'){ 
      for(j=nslavnode[i]; j<nslavnode[i+1]; j++){
	...
	if(contribution<-1.e-14 ||contribution>1.e-14){
	  printf("\tbdfill: node %" ITGFORMAT " sum Dinv*Dd+Dinv*Bd %e \n",nodesf,contribution);
	}
      }
    }
  }
  ...
  for(i=0;i<*nk;i++){
	...
	if ((idofs>0)&&(idofm>0)&&(contribution>1e-18 ||contribution<-1e-18)){ //insertion for active dofs  	      				
	  insertas(&irowdd, &mast1, &idofs, &idofm, &ifree, &nzsdd,
		   &contribution, &audd);				        
	}else if((idofs>0)&&(idofm<=0)&&(contribution>1e-18 ||contribution<-1e-18)){// mpc on master node			                  
	...
	}else if((idofm>0)&&(idofs<=0)&&(contribution>1e-18 ||contribution<-1e-18)){//mpc on slave node			                  
  ...
  printf("\tbdfill: \tsize audd %" ITGFORMAT " \n",icounter);
  nzsdd=icounter;
  RENEW(irowdd,ITG,nzsdd);
  RENEW(audd,double,nzsdd); 
  ...
  *nzsbd=ifree-1;
  ...
  printf("\tbdfill: \tsize aubd %" ITGFORMAT " \n",icounter);
  *nzsbd=icounter;
  RENEW(irowbd,ITG,*nzsbd);
  RENEW(aubd,double,*nzsbd);   
  ...
  printf("\tbdfill: \tsize auddtil2 %" ITGFORMAT " \n",icounter);
  nzsddtil2=icounter;
  RENEW(irowddtil2,ITG,nzsddtil2);
  RENEW(auddtil2,double,nzsddtil2); 
  ...
  ifree=1;
  /*nzsddtil=3*nzsd;
  RENEW(auddtil,double,nzsddtil);
  RENEW(irowddtil,ITG,nzsddtil);
  NNEW(mast1,ITG,nzsddtil);*/
  *nzsbdtil2=3*nzsbhelp;
  ...
  for(i=0;i<*nk;i++){
	...
	if ((idofs>0)&&(idofm>0)){ //insertion for active dofs  	      				
	  ...
	  if(imn<0){
	    // hier muss mpc2 rein, da Slavemittelknoten auf ind seite vorkomment können
		...
		while(index!=0){				   				            
		  ...
		  //if(idofm>0&&(c2>1e-14 ||c2<-1e-14)){
		  if(idofm>0){
		    if(debug==1)printf("\t\t node %" ITGFORMAT " dir %" ITGFORMAT " c %e \n",node1,dirind,coefmpc[index-1]);
		    if(debug==1)printf("\t\t idofs %" ITGFORMAT " idofm %" ITGFORMAT " c2 %e \n",idofs,idofm,c2);
		    insertas(&irowbdtil2, &mast1, &idofs, &idofm, &ifree, nzsbdtil2,
			     &c2, &aubdtil2);					    
		  }                                            
  ...
  *nzsbdtil2=ifree-1;
  printf("\tbdfill: \tsize aubdtil2 %" ITGFORMAT " \n",*nzsbdtil2);
  RENEW(irowbdtil2,ITG,*nzsbdtil2);
  RENEW(aubdtil2,double,*nzsbdtil2);
  debut=clock();
  dim=neq[1];
  ...
  for( i=0; i<*ntie; i++){    
	...
	if(islavact[j]>-1){
	  for(ll=0; ll<3; ll++){	  				
	    ...
	    dloc[9*j+ll*3+ll]=1.0; // fix if ndof<3
	    if(idofm>-1){
	    for(k=jqbdtil2[idofm]-1;k<jqbdtil2[idofm+1]-1;k++){
	      if(irowbdtil2[k]==nactdof[mt*(nodesf-1)+0+1]){//dof1
		dloc[9*j+ll*3+0]=aubdtil2[k];
	      }else if(irowbdtil2[k]==nactdof[mt*(nodesf-1)+1+1]){//dof2
		dloc[9*j+ll*3+1]=aubdtil2[k];
	      }else if(irowbdtil2[k]==nactdof[mt*(nodesf-1)+2+1]){//dof3
	  ...
	  printf(" bdfill:node %"ITGFORMAT", det(Ddloc)==0!\n Stop!\n",nodesf);
	  fflush(stdout); 
	  FORTRAN(stop,());
	  }
	  dinvloc[9*j+0]=+1.0/detdloc*(dloc[9*j+4]*dloc[9*j+8]-dloc[9*j+7]*dloc[9*j+5]);
	  dinvloc[9*j+1]=-1.0/detdloc*(dloc[9*j+1]*dloc[9*j+8]-dloc[9*j+7]*dloc[9*j+2]);
	  dinvloc[9*j+2]=+1.0/detdloc*(dloc[9*j+1]*dloc[9*j+5]-dloc[9*j+4]*dloc[9*j+2]);
	  dinvloc[9*j+3]=-1.0/detdloc*(dloc[9*j+3]*dloc[9*j+8]-dloc[9*j+6]*dloc[9*j+5]);
	  dinvloc[9*j+4]=+1.0/detdloc*(dloc[9*j+0]*dloc[9*j+8]-dloc[9*j+6]*dloc[9*j+2]);
	  dinvloc[9*j+5]=-1.0/detdloc*(dloc[9*j+0]*dloc[9*j+5]-dloc[9*j+3]*dloc[9*j+2]);
	  dinvloc[9*j+6]=+1.0/detdloc*(dloc[9*j+3]*dloc[9*j+7]-dloc[9*j+6]*dloc[9*j+4]);
	  dinvloc[9*j+7]=-1.0/detdloc*(dloc[9*j+0]*dloc[9*j+7]-dloc[9*j+6]*dloc[9*j+1]);
	  dinvloc[9*j+8]=+1.0/detdloc*(dloc[9*j+0]*dloc[9*j+4]-dloc[9*j+3]*dloc[9*j+1]);
	  //add to auddinv
	  for(ll=0; ll<3; ll++){
	    ...
	    for(k=0;k<3;k++){
	      ...
	      if(idofs>0 && idofm>0 && (contribution>1.e-18 || contribution<-1.e-18)){	
	  ...
	  //printf("\tbdfill: \tsize auddinv %" ITGFORMAT " \n",ifree-1);
	}
      }
    }
  }
  ...
  *nzsbdtil=*nzsbdtil2;
  ...
  //NNEW(mast1,ITG,*nzsbdtil);  
  jqbdtil[0]=1;
  for(i=0;i<neq[0];i++){
    //if(jqbdtil2[i+1]-jqbdtil2[i]!=0)printf("\t\t c %d jq %d %d \n",i+1,jqbdtil2[i]-1,jqbdtil2[i+1]-1);
   for(j=jqbdtil2[i]-1;j<jqbdtil2[i+1]-1;j++){
      idofs=irowbdtil2[j];
      //printf("\t \t irow %d",irowbdtil2[j]);
      idofm=i+1;
      //if(islavactdof[idofm-1]<0 || (islavactdof[idofm-1]>0 && islavact[(ITG) floor(islavactdof[idofm-1]/10.)-1]<0) || islavactdof[idofm-1]==0){ // only Mast Dofs and possibly N dofs due to mpcs
      if(islavactdof[idofm-1]<0 || (islavactdof[idofm-1]>0 && islavact[(ITG) floor(islavactdof[idofm-1]/10.)-1]<0) ){ // only Mast Dofs and possibly N dofs due to mpcs
	if(islavactdof[idofs-1]<1 && islavactdof[idofm-1]>0 ){
	 printf("bdfill: something went wrong in aubdtil2! Stop!\n");
	 printf("bdfill: noder %" ITGFORMAT " %" ITGFORMAT " nodec %" ITGFORMAT " %" ITGFORMAT " \n",islavnode[(ITG) floor(islavactdof[idofs-1]/10.)-1],idofs,islavnode[(ITG) floor(islavactdof[idofm-1]/10.)-1],idofm );
	 FORTRAN(stop,());
	}else if(islavactdof[idofs-1]<1 && islavactdof[idofm-1]<0 ){
	 printf("bdfill: something went wrong in aubdtil2! Stop!\n");
	 printf("bdfill: slavenode row   %" ITGFORMAT " %" ITGFORMAT " master node column %" ITGFORMAT " %" ITGFORMAT " \n",islavnode[(ITG) floor(islavactdof[idofs-1]/10.)-1],idofs,imastnode[-((ITG) floor(islavactdof[idofm-1]/10.))-1],idofm );
	 FORTRAN(stop,()); 
	}else if(islavactdof[idofs-1]<1 && islavactdof[idofm-1]==0 ){
	 printf("bdfill: something went wrong in aubdtil2! Stop!\n");	
	 printf("bdfill: slavenode row   %" ITGFORMAT " %" ITGFORMAT " master node column %" ITGFORMAT " \n",islavnode[(ITG) floor(islavactdof[idofs-1]/10.)-1],idofs,idofm );
	 FORTRAN(stop,());
	}
	...
	//printf("node %"ITGFORMAT" jrow %"ITGFORMAT" is %"ITGFORMAT" im %"ITGFORMAT"  \n",nodesf,jrow,idofs,idofm);
	iadd=0;
        if(jrow==1){// k=idof1
	    //iadd=0;
	    e1=aubdtil2[j];
	    //if(debug==1){printf("");}
	    if(j+1<jqbdtil2[i+1]-1 && irowbdtil2[j+1]==idof2){e2=aubdtil2[j+1];++iadd;}else{e2=0.0;}
	    if(j+1<jqbdtil2[i+1]-1 && irowbdtil2[j+1]==idof3){e3=aubdtil2[j+1];++iadd;}else if(j+2<jqbdtil2[i+1]-1 && irowbdtil2[j+2]==idof3){e3=aubdtil2[j+2];++iadd;}else{e3=0.0;}
	  }else if(jrow==2){// k=idof2
	    //iadd=0;
	    e1=0.0;
	    e2=aubdtil2[j];
	    if(j+1<jqbdtil2[i+1]-1 && irowbdtil2[j+1]==idof3){e3=aubdtil2[j+1];++iadd;}else{e3=0.0;}
	  }else{// k=idof3
	    //iadd=0;
	    e1=0.0;
	    e2=0.0;
	  ...
	  //printf("\te %e %e %e \n",e1,e2,e3);
	  contribution=dinvloc[9*(islavnodeentry-1)+0]*e1+dinvloc[9*(islavnodeentry-1)+3]*e2+dinvloc[9*(islavnodeentry-1)+6]*e3;
	  if(idof1>0 && (contribution>1.e-18 || contribution<-1.e-18)){
	  //printf("\tc %e dof %"ITGFORMAT" %"ITGFORMAT"  d %e %e %e \n",contribution,idof1,idofm,dinvloc[9*(islavnodeentry-1)+0],dinvloc[9*(islavnodeentry-1)+3],dinvloc[9*(islavnodeentry-1)+6]);
          insertas_ws(&irowbdtil,&idof1,&idofm,&ifree,
		      nzsbdtil,&contribution,&aubdtil);
	  }
	  contribution=dinvloc[9*(islavnodeentry-1)+1]*e1+dinvloc[9*(islavnodeentry-1)+4]*e2+dinvloc[9*(islavnodeentry-1)+7]*e3;
	  if(idof2>0 &&(contribution>1.e-18 || contribution<-1.e-18)){
	    //printf("\tc %e dof %"ITGFORMAT" %"ITGFORMAT" d %e %e %e \n",contribution,idof2,idofm,dinvloc[9*(islavnodeentry-1)+1],dinvloc[9*(islavnodeentry-1)+4],dinvloc[9*(islavnodeentry-1)+7]);
          insertas_ws(&irowbdtil,&idof2,&idofm,&ifree,
		      nzsbdtil,&contribution,&aubdtil);
	  }
	  contribution=dinvloc[9*(islavnodeentry-1)+2]*e1+dinvloc[9*(islavnodeentry-1)+5]*e2+dinvloc[9*(islavnodeentry-1)+8]*e3;
	  if(idof3>0 && (contribution>1.e-18 || contribution<-1.e-18)){
	    //printf("\tc %e dof %"ITGFORMAT" %"ITGFORMAT" d %e %e %e \n",contribution,idof3,idofm,dinvloc[9*(islavnodeentry-1)+2],dinvloc[9*(islavnodeentry-1)+5],dinvloc[9*(islavnodeentry-1)+8]);
  ...
  printf("\tbdfill: \tsize aubdtil %" ITGFORMAT " \n",ifree-1);
  *nzsbdtil=ifree-1;
  RENEW(irowbdtil,ITG,*nzsbdtil);
  RENEW(aubdtil,double,*nzsbdtil); 
  ...
  *nzsbdtil2=3*nzsbpgtil+3*nzsdpgtil;
  ...
  for(i=0;i<*nk;i++){
    for(j=jqdpgtil[i]-1;j<jqdpgtil[i+1]-1;j++){
      ...
      imn=islavnodeinv[i];
      if(debug==1)printf("auddpgtil node %" ITGFORMAT " %" ITGFORMAT " c %e \n",nodesf,nodem,contribution);
      for(ll=0; ll<3; ll++){	  				
	idofs = nactdof[mt*(nodesf-1)+ll+1];	    				
	idofm = nactdof[mt*(nodem-1)+ll+1];					
	if(debug==1)printf("\t idofs %" ITGFORMAT " idofm %" ITGFORMAT " \n",idofs,idofm);	    				
  ...
  if(debug==1){
	...
	for(j=nslavnode[i]; j<nslavnode[i+1]; j++){
	  ...
	  for(ll=0;ll<3;ll++){
	    ...
	    if(contribution<-1.e-10 ||contribution>1.e-10){
	      printf("\tbdfill: node %" ITGFORMAT " dof %" ITGFORMAT " sum Dd+Bd %e \n",nodesf,idofs,contribution);
	    }
	    }
	  }
	}
	...
	for(j=nslavnode[i]; j<nslavnode[i+1]; j++){
	  ...
	  for(ll=0;ll<3;ll++){
	    idofs = nactdof[mt*(nodesf-1)+ll+1];
	    if(idofs>0){
              if(islavact[j]>-1){contribution=1;}else{contribution=0.0;}
	      for(ii=0;ii<neq[1];ii++){
	    ...
	    if(contribution<-1.e-10 ||contribution>1.e-10){
	      printf("\tbdfill: node %" ITGFORMAT " dof %" ITGFORMAT " sum Dinv*Dd+Dinv*Bd %e \n",nodesf,idofs,contribution);
	    }
	    }
	  }
	}
