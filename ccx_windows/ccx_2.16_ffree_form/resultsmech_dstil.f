 ...
 !
 ! > \brief subroutine to calculate internal forces with transformed basis functions
 ...
 ! > @param [out] fn              internal forces
 ! > @param [in] iout              flag indicating what to calculate in results
 ! > @param [in] qa              q contains the contributions to the nodal force in the nodes belonging to the element at stake from other elements (elements already treated).
 ...
 ! > @param [in] calcul_fn       flag indicating whether to calculate internal forces
 ! > @param [in] calcul_qa       flag indicating whether to calculate qa
 ! > @param [in] calcul_cauchy       falg indicating whether to calculate the cauchy stress
 ! > @param [in] iener              flag indicating whether internal energy is calculated
 ! > @param [in] ikin              flag indicating whether to calculate the kinetic energy
 ! > @param [in] nal              used for qa
 ! > @param [in] ne0              used in spring elements
 ...
 ! > @param [in] nea              starting element index
 ! > @param [in] neb              end element index
        ...
        if(islavelinv(i).gt.0)then
         ...
         if(nope.eq.20 .or. nope.eq.10 .or. nope.eq.15)then
          ...
          enddo
            ...
            !          if(debug)write(*,*)'point',jj
            !          do i1=1,nope
            !           if(debug)write(*,*) i1,'shp',shp(1,i1),shptil(1,i1)
            !           if(debug)write(*,*) i1,'shp',shp(2,i1),shptil(2,i1)
            !           if(debug)write(*,*) i1,'shp',shp(3,i1),shptil(3,i1)
            !      &      shptil(4,i1)-shp(4,i1)
            !           shp(1,i1)=shptil(1,i1)
            !           shp(2,i1)=shptil(2,i1)
            !           shp(3,i1)=shptil(3,i1)
            !           shp(4,i1)=shptil(4,i1)
            !           write(*,*)'point',kk,'n',i1,'shp',shp(4,i1)
            !          enddo
            !         mortar end
            !
